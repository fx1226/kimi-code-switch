// Node FileAccess adapter over authorized filesystem and panel-store primitives.
import { createHash } from "node:crypto";
import { join } from "node:path";
import { invokeCommand as invoke } from "../native";
import { getAppDataDir } from "../native/paths";
...
export const getSaveTransactionPath = (): string => join(getAppDataDir(), "pending-save-transaction.json");
let activeSaveTransactionHash: string | null = null;

export const serverFileAccess: FileAccess = {
  ...
  async completeSaveTransaction(): Promise<void> {
    if (activeSaveTransactionHash) {
      ...
      await invoke("remove_file", { path: getSaveTransactionPath() });
    }
...
export const getRestoreTransactionPath = (): string => join(getAppDataDir(), "pending-restore-transaction.json");
...
export async function beginRestoreTransaction(record: RestoreTransactionRecord): Promise<void> {
  activeRestoreTransactionHash = await invoke<string>("write_text_cas", {
    path: getRestoreTransactionPath(),
    content: JSON.stringify(record),
    expectedSha256: "",
  });
}
...
export async function completeRestoreTransaction(): Promise<void> {
  if (activeRestoreTransactionHash) {
    ...
    await removeFile(getRestoreTransactionPath());
  }
...
export async function recoverPendingRestoreTransaction(): Promise<{
  ...
  const document = await serverFileAccess.readText(getRestoreTransactionPath());
  if (document === null) return { recovered: false, action: "none" };
  let parsed: unknown;
  try {
    parsed = JSON.parse(document);
  } catch {
    const quarantinedPath = await invoke<string>("quarantine_journal", { path: getRestoreTransactionPath() });
  ...
  const textStatuses = await Promise.all(changedTextFiles.map(async (file) => {
    const current = await serverFileAccess.readText(file.path);
  ...
  for (const { file, status } of textStatuses) {
    ...
    try {
      ...
      await serverFileAccess.writeTextCas!(file.path, file.desiredContent, expectedSha256);
    } catch (error) {
...
export async function recoverPendingSaveTransaction(): Promise<{
  ...
  const document = await serverFileAccess.readText(getSaveTransactionPath());
  if (document === null) return { recovered: false, action: "none" };
  let parsed: unknown;
  try {
    parsed = JSON.parse(document);
    ...
    const quarantinedPath = await invoke<string>("quarantine_journal", { path: getSaveTransactionPath() });
    return { recovered: false, action: "quarantined", reason: "malformed", quarantinedPath };
  }
  ...
  if (statuses.length === 0 || statuses.every((status) => status === "original")) {
    await removeFile(getSaveTransactionPath());
    return { recovered: true, action: "rollback" };
  }
  if (statuses.every((status) => status === "desired")) {
    await removeFile(getSaveTransactionPath());
  ...
  for (const { file, status } of textStatuses.reverse()) {
    ...
    try {
      ...
      if (file.originalContent === null) {
        await serverFileAccess.removeTextCas!(file.path, expectedSha256);
      } else {
        await serverFileAccess.writeTextCas!(file.path, file.originalContent, expectedSha256);
      }
    } catch (error) {
      rollbackErrors.push(`${file.path}: ${error instanceof Error ? error.message : String(error)}`);
    }
...
export async function copyKimiCodeConfiguration(fromHome: string, toHome: string): Promise<void> {
  ...
  await serverFileAccess.ensureDir(toHome);
  for (const name of ["config.toml", "mcp.json", "tui.toml", "AGENTS.md"]) {
    const content = await serverFileAccess.readText(`${fromHome}/${name}`);
    if (content !== null) {
      const sanitized = name === "config.toml"
        ...
        : name === "mcp.json"
          ...
          : content;
      await serverFileAccess.writeText(`${toHome}/${name}`, sanitized);
    }
  }
  ...
  if (await pathExists(sourcePlugins)) {
    ...
    const installed = await serverFileAccess.readText(installedPath);
    if (installed !== null) {
      const resolvedTargetHome = await invoke<string>("resolve_home_path", { path: toHome });
      await serverFileAccess.writeText(
        installedPath,
