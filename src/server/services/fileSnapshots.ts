// 文件指纹/快照服务：Node SHA-256、文件状态与统一文件访问。
//
...
async function readManagedDocument(id: ManagedFileId, path: string): Promise<string | null> {
  ...
  return serverFileAccess.readText(path);
}
...
export async function fingerprintFile(id: ManagedFileId, path: string): Promise<FileFingerprint> {
  if (id === "panel") {
    ...
    return { id, path, exists: true, size: Buffer.byteLength(content, "utf8"), mtimeMs: 0, sha256: sha256Hex(content) };
  }
  ...
  const content = (await readManagedDocument(id, path)) ?? "";
  return { id, path, exists: true, size: stat.size, mtimeMs: stat.mtime_ms, sha256: sha256Hex(content) };
}
