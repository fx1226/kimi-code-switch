...
vi.mock("../events", () => ({ onServerEvent: vi.fn() }));
vi.mock("../officialValidation", () => ({ verifyOfficialExecutable: vi.fn(async () => ({ status: "unavailable", version: null, diagnostics: [] })) }));
...
beforeEach(() => {
  ...
  vi.mocked(onServerEvent).mockReset();
...
describe("getCliVersion", () => {
  it("preserves prerelease and build suffixes instead of unlocking the stable contract", async () => {
    mockedInvoke.mockResolvedValue(exec(0, "kimi-code 2.0.0-beta.1+build.7\n") as never);
    const result = await getCliVersion();
    expect(result.version).toBe("2.0.0-beta.1+build.7");
    expect(evaluateCliCompatibility(result)).toBe("unverified-version");
  });

  it("does not expose unrecognized subprocess output as a detected version", async () => {
    const secret = "Authorization: Bearer plain-sensitive-fixture";
    mockedInvoke.mockResolvedValue(exec(0, secret) as never);
    const result = await getCliVersion();
    expect(result.version).toBe("");
    expect(JSON.stringify(result)).not.toContain(secret);
  });

  it("reads version cache and region from the server KIMI_CODE_HOME", async () => {
    vi.stubEnv("KIMI_CODE_HOME", "/custom/kimi-home");
    mockedInvoke
      .mockResolvedValueOnce(exec(0, "kimi-code 2.0.0") as never)
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce("global")
      .mockResolvedValueOnce(http(200, JSON.stringify({ version: "2.0.0" })) as never);

    await getCliVersion({ checkLatest: true });

    expect(mockedInvoke).toHaveBeenCalledWith("read_text", { path: "/custom/kimi-home/updates/latest.json" });
    expect(mockedInvoke).toHaveBeenCalledWith("read_text", { path: "/custom/kimi-home/region" });
  });
  ...
  it("bounds manual latest-version checks with a bounded timeout", async () => {
    mockedInvoke
      .mockResolvedValueOnce(exec(0, "kimi-code 1.0.0") as unknown as never)
...
describe("upgradeKimiCli / runKimiMcpServerTest", () => {
  it("forwards server login events and removes its listener after success", async () => {
    const onEvent = vi.fn();
    const unsubscribe = vi.fn();
    vi.mocked(onServerEvent).mockImplementation((name, handler) => {
      expect(name).toBe("kimi-oauth-login");
      handler({ kind: "start", target: "kimi-code" });
      return unsubscribe;
    });
    mockedInvoke.mockResolvedValue(exec(0, " logged in \n") as never);

    await expect(startKimiOAuthLogin("kimi-code", onEvent)).resolves.toEqual({
      ok: true,
      stdout: "logged in",
      stderr: "",
    });
    ...
    expect(mockedInvoke.mock.calls.map(([command]) => command)).toEqual(["start_kimi_oauth_login"]);
  });

  it("removes its server event listener when the login fails", async () => {
    const unsubscribe = vi.fn();
    vi.mocked(onServerEvent).mockReturnValue(unsubscribe);
    mockedInvoke.mockResolvedValue(exec(1, "", "login rejected") as never);

    await expect(startKimiOAuthLogin("kimi-code", vi.fn())).rejects.toThrow("login rejected");
  ...
  it("upgrades via Homebrew and trims output", async () => {
    mockedInvoke.mockResolvedValueOnce(exec(0, "kimi-code 2.0.0\n") as never)
      .mockResolvedValue(exec(0, " done \n", " warn \n") as never);
  ...
  it("upgrades kimi-code via Homebrew on non-Windows platforms", async () => {
    mockedInvoke.mockResolvedValueOnce(exec(0, "kimi-code 2.0.0\n") as never)
      .mockResolvedValue(exec(0, "updated", "") as never);
  ...
  it("parses tools from stdio MCP endpoints with an extended timeout", async () => {
    ...
    const result = await listKimiMcpServerTools("filesystem", {
      ...
      env: { KIMI_CODE_HOME: "/isolated/kimi" },
      extra: { cwd: "/isolated/project" },
    });
...
describe("runKimiConnectivityTest", () => {
  ...
  it("does not treat managed OAuth credential presence as verified connectivity", async () => {
    ...
    await expect(runKimiConnectivityTest(state, "p/m"))
      .rejects.toThrow(/Managed OAuth connectivity is unverified/);
  ...
  it("does not read default credential slots for an isolated managed OAuth environment", async () => {
    ...
    await expect(runKimiConnectivityTest(state, "p/m"))
      .rejects.toThrow(/active KIMI_CODE_HOME/);
    const [health] = await runProvidersHealthCheck(state);
    expect(health.reason).toBe("oauth-unverified");
    expect(mockedInvoke).not.toHaveBeenCalled();
  });
...
describe("evaluateCliCompatibility", () => {
  it("returns unknown when not installed", () => {
    expect(evaluateCliCompatibility({ version: "", installed: false })).toBe("unknown-version");
  });

  it("keeps an unrecognized installed release unverified", () => {
    expect(evaluateCliCompatibility({ version: "dev", installed: true })).toBe("unverified-version");
  });

  it("does not claim older releases match the native contract", () => {
    expect(evaluateCliCompatibility({ version: "0.9.0", installed: true })).toBe("unverified-version");
  });

  it("requires an exact contract release and does not assume newer releases are compatible", () => {
    expect(MIN_CLI_VERSION).toBe("2.0.0");
    expect(evaluateCliCompatibility({ version: MIN_CLI_VERSION, installed: true })).toBe("contract-matched");
    expect(evaluateCliCompatibility({ version: "9.9.9", installed: true })).toBe("unverified-version");
    expect(evaluateCliCompatibility({ version: "2.0.0-beta.1", installed: true })).not.toBe("contract-matched");
  });
});
