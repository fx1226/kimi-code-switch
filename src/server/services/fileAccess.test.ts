...
describe("copyKimiCodeConfiguration", () => {
  ...
  it("forwards atomic file operations to the native shell", async () => {
    ...
    await expect(serverFileAccess.readText("/a")).resolves.toBe("body");
    await serverFileAccess.writeText("/a", "next");
    await expect(serverFileAccess.writeTextCas!("/a", "next", "old")).resolves.toBe("new-hash");
    await serverFileAccess.removeTextCas!("/a", "new-hash");
    await serverFileAccess.ensureDir("/dir");
    await removeFile("/a");
    await moveFile("/a", "/b");
    await removeDir("/dir");
  ...
  it("handles absent journals and quarantines malformed/unsupported journal JSON", async () => {
    ...
    mockedInvoke
      ...
      .mockResolvedValueOnce("~/.kimi-code-switch/quarantine/pending-save-transaction.json.abc123" as never);
    ...
    mockedInvoke
      ...
      .mockResolvedValueOnce("~/.kimi-code-switch/quarantine/pending-save-transaction.json.def456" as never);
    await expect(recoverPendingSaveTransaction()).resolves.toMatchObject({
      recovered: false,
      action: "quarantined",
      reason: "unsupported",
